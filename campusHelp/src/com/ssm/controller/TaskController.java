...
public class TaskController {
	...
	public String newtask(Task task, HttpServletRequest request, Model model) {
		...
		try {
			...
			if (user.getHierarchy()<4){
				model.addAttribute("msg", "发布失败-用户信誉等级低于4");
				return "userNewtask";
			}
		} catch (Exception e) {
		...
		if (ruser> 0) {
			...
			if (r > 0) {
				...
				/**
				 * 将发布成功的任务信息写入到evaluation表中
				 */
				MyEvaluation myEvaluation = new MyEvaluation();
				myEvaluation.setIsEvaluation(CommonProperties.notEvaluation.getAmount());
				myEvaluation.setState(0);
				myEvaluation.setTaskid(task2.getTaskid());
				evaluationService.insertEvaluation(myEvaluation);
	...
	public String getuseratask(HttpServletRequest request, Model model) {
		...
		List<MyEvaluation> myEvaluationsList=new LinkedList<>();
		for (Task task:list ) {
			MyEvaluation myEvaluation = new MyEvaluation();
			myEvaluation.setTask(task);
			myEvaluationsList.add(myEvaluation);
		}

//		查询对应任务的发布者评价内容
		for (MyEvaluation evaluation:myEvaluationsList) {
			int taskid = evaluation.getTask().getTaskid();
			MyEvaluation myEvaluation =evaluationService.getEvaluationByTaskid(taskid);
			evaluation.setIsEvaluation(myEvaluation.getIsEvaluation());
			evaluation.setTaskid(myEvaluation.getTaskid());
			evaluation.setContent(myEvaluation.getContent());
			evaluation.setHierarchy(myEvaluation.getHierarchy());
			evaluation.setState(myEvaluation.getState());
		}

		model.addAttribute("list", myEvaluationsList);
		return "userAtask";
	}
	...
	public String taskok(String tidstr, HttpServletRequest request, Model model) {
		...
		if (ruser>0) {
			...
			//修改评价任务表的任务状态
			evaluationService.updateStateByTaskId(tidstr);
			if (r > 0) {
