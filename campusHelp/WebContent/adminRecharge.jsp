						...
						<fieldset class="layui-elem-field">
							...
							<c:forEach items="${list }" var="recharge">
								<div style="margin-left: 20px;margin-right: 30px;margin-top: 20px;">
									<c:if test="${recharge.isRecharge==0 }">
										<span class="layui-badge layui-bg-blue">待充值</span>
									</c:if>
									<c:if test="${recharge.isRecharge==1 }">
										<span class="layui-badge layui-bg-gray">已充值</span>
									</c:if>
									<c:if test="${recharge.isRecharge==2 }">
										<span class="layui-badge layui-bg-gray">已拒绝</span>
									</c:if>
								</div>
								<fieldset class="layui-elem-field layui-field-title"
									style="margin: 20px;margin-bottom: 5px;margin-top: 5px;">
									<legend><a>订单号：${recharge.orderId }</a></legend>
								</fieldset>
								<div style="margin-left: 20px;margin-right: 30px;">
									<span class="layui-badge layui-bg-gray">详细描述</span>
									<span class="layui-badge layui-bg-gray">${recharge.name }</span>
									<span class="layui-badge layui-bg-gray">${recharge.studentid }</span>
									<span class="layui-badge layui-bg-gray">${recharge.school }</span>
									<span class="layui-badge layui-bg-gray">充值数额：${recharge.amount }</span>
								</div>
								<div style="margin-left: 30px;margin-right: 30px;margin-top: 10px;margin-bottom: 30px;">
									<c:if test="${recharge.isRecharge==0 }">
										<a href="admin/agreeRecharge.do?orderId=${recharge.orderId }&name=${recharge.name }&studentid=${recharge.studentid }&amount=${recharge.amount }&schoolid=${recharge.schoolid}&school=${recharge.school}" class="layui-btn layui-btn-sm layui-btn-primary layui-btn-radius">同意</a>
										<a href="admin/refuseRecharge.do?orderId=${recharge.orderId }&name=${recharge.name }&studentid=${recharge.studentid }&amount=${recharge.amount }&schoolid=${recharge.schoolid}&school=${recharge.school}" class="layui-btn layui-btn-sm layui-btn-primary layui-btn-radius">拒绝</a>
									</c:if>
								</div>
